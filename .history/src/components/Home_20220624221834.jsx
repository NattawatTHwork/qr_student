...
const Home = () => {
  ...
  useEffect(() => {
    ...
    onSnapshot(q, (QuerySnapshot) => {
      setStudents(QuerySnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id, sid: doc.data().sid, firstname: doc.data().firstname, lastname: doc.data().lastname, time: doc.data().time.tostring() })))
    });
  }, [])
              ...
              <tbody>
                {students.map((student) => (
                  <tr key={student.id}>
                    ...
                    <td className='border-2'><p className='ml-2 mt-2 mr-2 mb-2'></p></td>
                  </tr>
                ))}
              </tbody>
            </table>
