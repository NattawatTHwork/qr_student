...
import { collection , query , where , onSnapshot , addDoc , ser } from 'firebase/firestore'
...
const Scan = () => {
  ...
  const writeList = async (user) => {
    await addDoc(collection(db, 'list'), { sid: user.sid, firstname: user.firstname, lastname: user.lastname, time: Timestamp() })
    setUser(false)
  }
